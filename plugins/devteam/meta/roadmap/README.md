...
| [0.2](0.2/README.md) | 0.2.0 | **structural containment and the queue the run left**: workers run inside a per-worker copy-on-write sandbox and reach the host only through a promotion gate; the manager rotates instead of compacting; the checks are audited against the rule each one enforces; the rule set is swept for pairs that cannot both hold; then a second run that exercises `iterate`, rotation and the unreviewed-decision path | planned 2026-09-05; **0.2.0 done 2026-09-07** — the composition, the headless worker inside it and the containment of git history all `MEASURED`. **0.2.1 to 0.2.8 and 0.2.10 done; 0.2.9, the second run, STOPPED 2026-09-19** by the owner after both its cycles ran — read whole in its Findings, and sorted for v3 in [`../v3-findings-register-2026-09-19.md`](../v3-findings-register-2026-09-19.md) |
| [0.3](0.3/README.md) | 0.3.0 | **evidence the pipeline can trust, handoffs that carry, and a run that closes**: the second run's 142 findings, taken theme by theme — items carried to a disposition, checks that say when they did not look, the manager's bookkeeping made commands, meters that see what the owner's gauge sees — then the handoff, the sandbox frozen at a commit, the adversarial layer, signed text and iterate; the liaison layer built only if a probe says the platform can carry it; then a third run | **planning since 2026-09-23** — the map approved by the owner that day, with fourteen decisions settled (its §6, L-1 to L-14) and the third run targeted at a cowsay clone in Nitpick. **[0.3.0](done/0.3.0.md), the probes, done 2026-09-24**, adding L-15 to L-18; **[0.3.1](done/0.3.1.md), the check contract, done 2026-09-24**, adding the commit gate and its hook, and P-50 and P-51; **[0.3.2](done/0.3.2.md), what the checks read, done 2026-09-24**, taken before 0.3.3 and adding P-48b, `claim.py` and `tally.py`; **[0.3.3](0.3/0.3.3.md), the disposition ledger, planned 2026-09-24**, next on the critical path, with the owner's six answers recorded before any step is built |
