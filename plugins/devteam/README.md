...
- **0.2.9 — the second run.** Cycle 2 on the fixture through
  `/devteam:iterate`, with a client briefed to be slow on reversible
  questions so P-27 fires for the first time, workers sandboxed at width
  2–3, rotation at a checkpoint, and a budget ceiling that blocks rather
  than warns. **This is what the cycle was for.** Every mechanism 0.2 built
  is currently proved by its own controls and by one walk on a throwaway
