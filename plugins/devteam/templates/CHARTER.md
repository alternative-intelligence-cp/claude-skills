...
| Protected paths | <trees the pipeline may read but never write — vendored deps, generated output, sibling repos. **One path per entry, comma-separated, and nothing else in this cell.** The guard splits this cell on commas and strips backticks, so a comma inside a sentence becomes two entries and each becomes a path that matches nothing — the row then reads correctly to every human who checks it and protects NOTHING. Write `` `dist/, vendor/` ``, never a description of them. `none` is a legitimate value. `check_trace` reports an entry that is not a single path> |
...
**Carried by D-<n>. Answers Q-<n>. Signed by the client.**

**What changed.** <one line>

**Why.** <the measured reason>

**Superseded text, kept per P-23:**
