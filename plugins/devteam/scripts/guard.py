...
def judge(target, what, session, session_project, cache, category="write"):
    ...
    return (f"Refused: {what} to a path no live task has claimed, while {running} "
            ...
            "gives you the committed tree to mutate and the guard does not "
            "police it. Remove it afterwards with the SAME ABSOLUTE PATH \u2014 "
            "a relative path or a bare glob resolves against your working "
            "directory, which may be inside a project, and is judged there "
            "rather than where you meant it. A verifier that treats "
