...
| `bad-status` | `FORMATS.md` §"Status vocabularies" | a written status value, a `Status.` or `Class.` field, or a ledger entry's `Disposition.`, each read whole across its continuation lines ↔ its closed set. A ledger disposition's message shows the vocabulary | `enforces` |
